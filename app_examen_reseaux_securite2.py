...
Partie B : Scénarios de configuration Cisco CLI interactif (20 pts)

Exécution : streamlit run app_examen_reseaux_securite.py
"""
...
st.set_page_config(
    page_title="Examen Réseaux & Sécurité Cisco — 40 pts",
    page_icon="🛡️",
    layout="wide",
)
...
def simulate_cli_command(state: Dict[str, Any], command: str, scenario: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    if c in {"exit", "logout"}:
        ...
        if mode == "privileged":
            state["mode"] = "user"
        elif mode in {"interface", "vlan", "router"}:
            state["mode"] = "config"
        elif mode == "config":
            state["mode"] = "privileged"
        return True, ""

    if c in {"end", "^z"} and mode in {"config", "interface", "vlan", "router"}:
        state["mode"] = "privileged"
        return True, ""
    ...
    if c.startswith("do show ") and mode in {"config", "interface", "vlan", "router"}:
        return True, fake_show_output(c, scenario)
...
st.markdown(
    '<div class="subtitle">Barème total : <b>40 points</b> — Questions de cours + scénarios de configuration en mode commande Cisco interactif.</div>',
    unsafe_allow_html=True,
)
...
with st.sidebar:
    ...
    st.write("• Scénarios Cisco CLI : 20 pts")
    st.divider()
...
tab_course, tab_cli, tab_submit = st.tabs(["Partie A — Questions de cours / 20", "Partie B — Scénarios Cisco CLI / 20", "Correction et export"])
...
with tab_cli:
    st.subheader("Partie B — Scénarios de configuration Cisco IOS : 20 points")
    st.markdown('<div class="rubric">4 scénarios × 5 points. Utilisez le terminal interactif : chaque commande validée fait évoluer le prompt comme sur un équipement Cisco.</div>', unsafe_allow_html=True)
...
with tab_submit:
    ...
    if submitted:
        ...
        st.markdown(
            f"""
            <div class="score-box">
                <h2>Score final : {total}/40</h2>
                <p><b>Questions de cours :</b> {course_total}/20 &nbsp; | &nbsp; <b>Scénarios Cisco CLI :</b> {cli_total}/20</p>
            </div>
            """,
            unsafe_allow_html=True,
        )
...
st.divider()
st.caption("Examen généré pour évaluer les bases de routage, switching/VLAN/STP, NAT, ACL Cisco, cryptographie et PKI. Barème total : 40 points.")
